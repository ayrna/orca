...
public:
  ...
  Solver_SVORIM(const int _nr_thres) { nr_thres = _nr_thres; th = new double[nr_thres];}
  ~Solver_SVORIM() { delete th; }
  void Solve(int l, const QMatrix& Q, const double *b, const schar *y,
	     double *alpha, double Cp, double Cn, double eps,
	     SolutionInfo* si, int shrinking){
    Solver::Solve(l,Q,b,y,alpha,Cp,Cn,eps,si,0); // don't do shrinking -- otherwise needs more complicated select_working_set
  }
...
int Solver_SVORIM::select_working_set(int &out_i, int &out_j)
{
	...
	for(int i=k;i<l;i+=nr_thres) //only consider things in the same group
	{
		if(y[i]==+1)	// y = +1
		{
...
// Q_{(i, k), (i', k')} = z[(i, k)] z[(i', k')] * { K(x[i], x[i']) + th_cost * [ k == k' ] }
...
{ 
public:
	RNK_Q(const svm_problem& prob, const svm_parameter& param, const int _nr_thres, const double _th_cost = 1)
	:Kernel(prob.l, prob.x, param)
	{
		l = prob.l;
		nr_thres = _nr_thres;
...
static void solve_c_rnk(
	const svm_problem *prob, const svm_parameter *param,
	double *alpha, Solver::SolutionInfo* si, double *thres, int nr_thres)
{
	int l = prob->l;
	int bigl = l * nr_thres;
...
static decision_function svm_train_one(
	const svm_problem *prob, const svm_parameter *param,
	double Cp, double Cn, int n_class = 2)
{
...
double svm_predict(const svm_model *model, const svm_node *x)
{
	   ...
	   model->param.svm_type == NU_SVR)
	{
		...
		if(model->param.svm_type == ONE_CLASS)
			return (res>0)?1:-1;
		else if (model->param.svm_type == C_RNK || model->param.svm_type == SVORIM){
...
{
	"c_svc","nu_svc","one_class","epsilon_svr","nu_svr","c_rnk","svorim",NULL
};

const char *kernel_type_table[]=
{
	"linear","polynomial","rbf","sigmoid","stump","perc","laplace","expo","precomputed",NULL
};
...
void svm_free_model_content(svm_model* model_ptr)
{
	...
	if(model_ptr->sv_coef)
	{
		...
		if ((model_ptr->param).svm_type != C_RNK && (model_ptr->param).svm_type != SVORIM){
			for(int i=0;i<model_ptr->nr_class-1;i++)
				free(model_ptr->sv_coef[i]);
		}else
			free(model_ptr->sv_coef[0]);
