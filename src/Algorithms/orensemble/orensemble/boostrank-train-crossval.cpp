...
int main (unsigned int argc, char* argv[]) {
  ...
  if(base/300!=1)
  {
	...
	set_seed(0);
 	...
 	cv.set_folds(5); // folds = 5

	/* set up different versions of the ORBoost */
	int vv[6]={5,10,15,20,30,40};
	for (UINT i = 0; i < 6; i++) {
		lemga::FeedForwardNN* p = new lemga::FeedForwardNN();
		lemga::NNLayer l(n_in, vv[i]);
		l.set_weight_range(-1, 1);
		p->add_top(l);
		lemga::NNLayer lout(vv[i],1);
		lout.set_weight_range(-1, 1);
		p->add_top(lout);
		p->initialize();
		p->set_train_method(lemga::FeedForwardNN::CONJUGATE_GRADIENT);
     		p->set_parameter(0.1, 1e-7, 200);
		
		lemga::AggRank *pbag_i = pbag->clone();
		if (bag % 10 == 1)
			pbag_i->set_reg_param(1e-32);
		else
			pbag_i->set_reg_param(0.0);
		pbag_i->set_base_model(*p);
		pbag_i->set_max_models(n_iter);
		pbag_i->reset();
	...
	cv.train();
 	...
 	pbag = (lemga::AggRank *) cv.best_model().clone();
	pbag->reset();
